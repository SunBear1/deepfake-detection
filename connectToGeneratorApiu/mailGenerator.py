...
class OneSecMail():
    ...
    def create_new_emails(self, numberOfEmails):
        self.mails = self.session.get(mailEndpoints.NEW_EMAILS + str(numberOfEmails)).json()        
    ...
    def update_messages(self):
        for idx in range(len(self.mails)):
            self.messages[idx] = self.session.get(mailEndpoints.CHECK_INBOXES.format(self.usernames[idx], self.domains[idx])).json()

    def update_last_messages(self):
        for idx in range(len(self.mails)):
            try:
                self.lastMessages[idx] = self.session.get(mailEndpoints.CHECK_EXACT_MESSAGES.format(self.usernames[idx], self.domains[idx], self.messages[idx][-1]['id'])).json()
    ...
    def update_attachments(self):
        for idx in range(len(self.mails)):
            try:
                self.lastMessagesAttachment[idx] = self.session.get(mailEndpoints.ATTACHMENT_DOWNLOAD.format(self.usernames[idx], self.domains[idx], self.messages[idx][-1]['id'], self.lastMessagesAttachmentName))
            except:
                pass
